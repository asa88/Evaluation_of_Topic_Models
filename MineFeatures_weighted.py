...
class MineFeatures_weighted(object):
    '''
    classdocs
    '''
    ...
    def loadEdges(self,edgeObj):
        for line in edgeObj:
            ...
            sib_key=int(temp[1])
            try:
                wght= round(1/float(temp[2]),2)
                self.G.add_edge(self.G_Id[key],self.G_Id[sib_key],weight=wght)
            except:
                pass
        return
        ...
        for node in self.gM.nodes():
            ...
            for item in neighbours:
                if self.gM.has_node(item):
                    try:
                        if node!=item:
                            wght=self.G[node][item]['weight']
                            self.gM.add_edge(node,item,weight=wght)
                    except:
                        pass
    ...
    def calc_SpanningFeatures(self,path,count):
        ...
        for source in self.gM.nodes():
            for target in self.gM.nodes():
                if source!=target and [source,target] not in closed:
                    path_len=nx.dijkstra_path_length(self.G, source, target)
                    sp_count+=1
                    '''
                    for item in path:
        ...
        for node in self.gS_w.nodes():
            ...
            for friend in friends:
                path=nx.dijkstra_path(self.G,node,friend)
                for i in range(0,len(path)-1):
                        wght=self.G[path[i]][path[i+1]]['weight']
                        self.gS.add_edge(path[i],path[i+1],weight=wght)     
        ...
        for edge in self.gS_w.edges(data=True):
         ...
         #add weight to edges    
        for edge in self.gS.edges(data=True):
            ed=Gs.get_edge(edge[0],edge[1])
            ed.attr['label']=edge[2]['weight']
        
         #add weight to edges    
        for edge in self.gM.edges(data=True):
            ed=Gm.get_edge(edge[0],edge[1])
            ed.attr['label']=edge[2]['weight']
    ...
    def genFeatures(self,topicsObj,path):
        ...
        for line in topicsObj:
            ...
            print "finished calculating Proj Features"
            self.calc_SpanningFeatures(path,count)
            print "finished calculating Spanning Features"
